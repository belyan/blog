...
Теперь заголовок `h2` можно расположить как после заголовка `h1`, так и после параграфа. В любом случае отступ сверху не будет превышать `20px`.

## Общие рекомендации

Подводя итог, я бы хотел перечислить общие правила, которых я придерживаюсь при расстановке полей и отступов. 

1. Если соседние элементы имеют одинаковые отступы, то задавать их родительскому контейнеру, а не элементам.
2. При задании отступов между элементами, следует учитывать является ли элемент обязательным или опциональным.
3. Для списка однотипных элементов, не забывать о том, что число элементов может варьироваться.
4. Помнить о "схлопывании" вертикальных отступов, и использовать его там, где оно принесет пользу.